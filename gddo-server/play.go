...
func (s *server) playURL(pdoc *doc.Package, id, countryHeader string) (string, error) {
	if m := exampleIDPat.FindStringSubmatch(id); m != nil {
		if e := findExample(pdoc, m[1], m[2], m[3]); e != nil && e.Play != "" {
			req, err := http.NewRequest("POST", ConfigPlaygroundURL+"/share", strings.NewReader(e.Play))
			if err != nil {
				return "", err
			}
			...
			if resp.StatusCode > 399 {
				return "", &httpError{
					...
					err:    fmt.Errorf("Error from %s: %s", ConfigPlaygroundURL, p),
				}
			}
			return fmt.Sprintf("%s/p/%s", ConfigPlaygroundURL, p), nil
		}
	}
